...
**Gyomu** は、**TSDocの生成、ディレクトリ/パッケージ単位の設計コンセプト抽出、多言語READMEの構築**を全自動化・維持する、リポジトリ完結型のオープンAIエージェントシステムです。

将来的には、テストコードの同期・補正など、コード変更に伴う関連資産の自動更新にも対応することを目指しています。
...
**ルート以外の各パッケージに含まれる TSDoc コメントや README.md は、Gyomu のドキュメント生成機能によって生成・継続的に管理されています。**

---

## 動作要件

Gyomuは、Google AI Studioの無料利用枠だけで開発・検証を始められるよう設計されています。
