...
export class ZipArchive extends AbstractBaseArchive {
      ...
      (acc, info) =>
        acc.andThen(() => {
          ...
          if (!platform.existsSync(sourcePath)) {
            return errAsync(new ArchiveError(`File Not Found: ${sourcePath}`));
          }
  ...
  extractSingileFile(
    ...
    if (this.password) {
      return errAsync(
        new ArchiveError('Password protected zip extraction is not supported'),
      );
    }
          ...
          return ResultAsync.fromPromise(
            ...
            (e) =>
              e instanceof ArchiveError
                ? e
                : new ArchiveError('Fail to extract file', e as Error),
          );
        })
        // ④ 最終結果
        .map(() => true)
