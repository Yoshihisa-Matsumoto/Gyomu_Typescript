import { Csv2Json, Json2Csv } from '../csv.js';

import { tmpDir, validateTextFiles } from './baseClass.js';
...
test('csv read test', async () => {
  ...
  dataResult = await Csv2Json('tests/test.utf8.bom.csv', {
    bom: true,
    filterFn: (data) => data['UserName'] == '西澤 智也',
  });
  if (dataResult.isErr()) {
    expect(dataResult.isOk()).toBeTruthy();
  } else {
...
test('csv write test', async () => {
  ...
  const result3 = await Json2Csv(testData, {
...
test('csv read/write compare test', async () => {
  ...
  const fileResult = await Json2Csv(data, {
...
import { rowsFromZip, rowsFromGzip } from '../csv.js';
