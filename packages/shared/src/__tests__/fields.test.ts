...
const decode = <A>(
  ...
  Effect.runPromise(
    convertToSchemaObjectWithEffect(schemaName)(schema, input) as Effect.Effect<
      A,
      never,
      never
    >,
...
const encode = <A>(schemaName: string, schema: Schema.Schema<A>, input: A) =>
  Effect.runPromise(
    convertFromSchemaObjectWithEffect(schemaName)(
      schema,
      input,
...
describe('schemaField.int', () => {
  ...
  it('異常系: 小数はNG', async () => {
    await expect(
      decode('decimal', schemaField.int(), 1.5),
    ).rejects.toBeTruthy();
  });
...
describe('schemaField.boolean', () => {
  ...
  it('異常系: stringはNG', async () => {
    await expect(
      decode('string ng', schemaField.boolean, 'true'),
    ).rejects.toBeTruthy();
  });
});
