...
test('db error test', () => {
  ...
  const TestLayer = ParameterServiceLayer.pipe(Layer.provide(GyomuRepositoryMock))
  const testRunner = makeRunner(TestLayer)
  const program = Effect.gen(function* () {
    const parameter = yield* ParameterService
    return yield* parameter.setValue('abc', 'abc')
  })
  expect(async () => await testRunner(program)).rejects.toThrow('DB connection error')
})
test('no parameter found', () => {
  const GyomuRepositoryMock = Layer.succeed(GyomuRepository, {
    parameterMaster: {
...
describe('multiple parameter with different value test', () => {
  ...
  const records: Array<typeof ParameterMasterSchema.types._select> = [
    {
      id: testId,
      itemKey: itemKey,
      itemValue: 'oldest',
  ...
  const TestRetrieveLayer = ParameterServiceLayer.pipe(Layer.provide(GyomuRepositoryRetrieveMock))
...
async function setValueTest<T extends string | boolean | number>(itemValue: T) {
  ...
  let TestLayer = ParameterServiceLayer.pipe(Layer.provide(GyomuRepositoryMock))
  ...
  let TestRetrieveLayer = ParameterServiceLayer.pipe(Layer.provide(GyomuRepositoryRetrieveMock))
  ...
  const programValue = Effect.gen(function* () {
    ...
    if (typeof itemValue === 'string') {
      ...
      return yield* Effect.fail(new DBError({ message: 'Invalid type', cause: undefined }))
    }
  })
  ...
  TestRetrieveLayer = ParameterServiceLayer.pipe(Layer.provide(GyomuRepositoryRetrieveMock))
