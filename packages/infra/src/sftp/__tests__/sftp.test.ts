...
import { Effect, Layer, Option, Redacted, Result, Stream } from 'effect'
...
describe('SftpService', () => {
  ...
  const usingLayer = Layer.provideMerge(SftpService.live, mockConfigService).pipe(
  ...
  describe('withConnection - basic functionality', () => {
    it('should establish FTP connection and execute function', async () => {
      const program = Effect.gen(function* () {
        ...
        return yield* sftp.withConnection('', () => Effect.succeed('connected'))
      })
  ...
  describe('getFileInfo functionality', () => {
    it('should get file info', async () => {
      const program = Effect.gen(function* () {
        const sftp = yield* SftpService
        return yield* sftp.withConnection('', (client) => client.getFileInfo('/test/file.txt'))
      })
  ...
  describe('download functionality', () => {
    it('should download file', async () => {
      ...
      readdir.mockImplementationOnce((path, cb) => {
        cb(null, [{ filename: 'file.txt', attrs: { isDirectory: () => false } }])
      })

      const program = Effect.gen(function* () {
        const sftp = yield* SftpService
        return yield* sftp.withConnection('', (client) => client.download(transportInfo))
      })
    ...
    it('should download directory', async () => {
      ...
      readdir.mockImplementationOnce((path, cb) => {
        cb(null, [{ filename: 'dir', attrs: { isDirectory: () => true } }])
      })
  ...
  describe('upload functionality', () => {
    it('should upload file', async () => {
      ...
      const program = Effect.gen(function* () {
        const sftp = yield* SftpService
        return yield* sftp.withConnection('', (client) => client.upload(transportInfo))
      })
  ...
  describe('error handling', () => {
    ...
    it('should handle config errors', async () => {
      ...
      const failureLayer = Layer.provideMerge(SftpService.live, failingConfigService).pipe(
        Layer.provide(NodeFileSystem.layer),
      )
