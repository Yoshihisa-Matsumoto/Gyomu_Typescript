...
const createControlledReadable = (chunks: Array<string>) => {
  let index = 0
...
describe('nodeStream', () => {
  describe('acquireNodeStream', () => {
    ...
    it('should handle creation errors', async () => {
      ...
      const effect = acquireNodeStream(create)
      await expect(Effect.runPromise(Effect.scoped(effect))).rejects.toThrow('creation failed')
    })
  })
  ...
  describe('throughNodeStream', () => {
    ...
    it('should apply backpressure via Queue (slow consumer)', async () => {
      ...
      const input = Stream.fromIterable(Array.from({ length: 200 }, (_, i) => i))
  ...
  describe('throughNodeStreamScoped', () => {
    ...
    it('should handle creation errors in scoped version', async () => {
      ...
      await expect(Effect.runPromise(Stream.runForEach(output, () => Effect.void))).rejects.toThrow(
        'creation failed',
      )
    })
