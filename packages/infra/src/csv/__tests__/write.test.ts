...
describe('writeCsv', () => {
  ...
  it('should apply custom options to CSV stringifier', async () => {
    ...
    // 実際に動かす設計図を作る
    const program = input.pipe(writeCsv(testSchema, options), Stream.runCollect)
  ...
  it('should write csv', async () => {
    ...
    const result = await Stream.runCollect(rows.pipe(writeCsv(schema))).pipe(Effect.runPromise)
