...
describe('CSV Read Functions', () => {
  describe('parseCsv', () => {
    it('should parse basic CSV data', async () => {
      ...
      const result = await Stream.runCollect(stream.pipe(parseCsv())).pipe(Effect.runPromise)
    ...
    it('should handle CSV with BOM option', async () => {
      ...
      const result = await Stream.runCollect(stream.pipe(parseCsv({ bom: true }))).pipe(
        Effect.runPromise,
      )
  ...
  describe('readCsv', () => {
    it('should read and validate CSV data', async () => {
      ...
      const result = await Stream.runCollect(stream.pipe(readCsv(schema))).pipe(Effect.runPromise)
  ...
  describe('readCsvRaw', () => {
    it('should read raw CSV data without validation', async () => {
      ...
      const result = await Stream.runCollect(stream.pipe(readCsvRaw())).pipe(Effect.runPromise)
