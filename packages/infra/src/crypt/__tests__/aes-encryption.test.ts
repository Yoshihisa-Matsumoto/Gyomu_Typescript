...
test('aes gcm decode compatibility with other library encoded data', () => {
  const csharp_result = 'h6QYCpU8SqaRBUeGFTO0esu4SNBUVlIYzEQkYh1W6j9qmgT0OoEZuUvj'
  const key = 'abc'
...
test('AES Encrypt/Decrypt using binary file key', async () => {
  ...
  const encryptedBuffer = await runNodeWithEnvOrThrow(
    aes.aesEncryptBufferByKeyFile(bufferToArrayBuffer(plainBuffer), keyFilename),
  )
...
test('PKI Encrypt/Decrypt using key pair file', async () => {
  ...
  const plainFilename = './tests/utf8_sample.txt'
  const encryptedFilename = getTempFilename()
  const decryptedFilename = getTempFilename()
  await runNodeWithEnvOrThrow(pki.pkiFileEncryptToFile(publicKey, plainFilename, encryptedFilename))
