...
const validateFileExistence = async (
  ...
  const program = (pathToStream: string, entryNameInTar: string) =>
    fileStream(pathToStream).pipe(existsInTar(entryNameInTar))
  const result = await runNodeWithEnv(program(tarFilename, entryName))
  if (Result.isSuccess(result)) {
    if (result.success !== expected_result) {
      console.log(entryName, 'Different from expected result:', expected_result)
    }
...
describe('untar test', () => {
        ...
        Stream.runCollect(entry.openStream()).pipe(
          Effect.map((chunks) => {
            ...
            if (entry.path === 'README.md') {
              const buffer = Buffer.concat(Array.from(chunks).map((c) => Buffer.from(c)))
  ...
  it('Tar Creation Test', async () => {
    ...
    await validateFileExistence(tarFileName, 'folder1/folder 2/aes_encryption.py', true)

    await validateFileExistence(tarFileName, 'folder1\\folder 2\\aes_encryption.py', true)

    await validateFileExistence(tarFileName, 'folder1\\folder 3\\aes_encryption.py', false)
    ...
    const program2 = Effect.gen(function* () {
      ...
      const tar = yield* TarService
      yield* fileStream(tarFileName).pipe(tar.extractDirectory({ targetDir: destinationRoot }))
      validateFolders(path.join(compressDirectory, 'source'), destinationRoot)
    })

    await runNodeWithEnvOrThrow(program2, TarService.live)
  ...
  it('Tar Unarchive Test', async () => {
    ...
    const program = (transferInformation2: FileTransportInfo) =>
      Effect.gen(function* () {
        const tar = yield* TarService
        const tarFileName = path.join(compressDirectory, 'compress/temp.tar')
        yield* fileStream(tarFileName).pipe(tar.extract(transferInformation2))
        return tarFileName
      })
    const result = await runNodeWithEnvOrThrow(program(transferInformation), TarService.live)
    const tarFileName = result
    ...
    await runNodeWithEnvOrThrow(program(transferInformation), TarService.live)
