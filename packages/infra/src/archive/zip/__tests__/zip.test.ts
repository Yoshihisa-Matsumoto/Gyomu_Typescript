...
describe('Zip resolvePath test', () => {
  it('resolvePath', () => {
    ...
    expect(output).toBe(join(tmpdir(), 'compressZip', 'extract', 'outputREADME.md'))
    ...
    expect(output).toBe(join(tmpdir(), 'compressZip', 'extract', 'email_sender.py'))
    ...
    expect(output).toBe(join(tmpdir(), 'compressZip', 'extract', 'folder 2', 'aes_encryption.py'))
    ...
    expect(output).toBe(join(tmpdir(), 'compressZip', 'extract', 'folder 2', 'folder4'))
  })
})
describe('Zip Test', () => {
  it('Dictionary Check', async () => {
    ...
    await runNodeWithEnvOrThrow(program('tests/compress/temp.zip'), ZipService.live)
  })
    ...
    const program = (filePath: string) =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          expect(
            yield* existsInZip(join('folder1', 'folder 2', 'aes_encryption.py'))(entries),
          ).toBeTruthy()
          expect(
            yield* existsInZip(join('folder1', 'folder 3', 'aes_encryption.py'))(entries),
          ).toBeFalsy()
  ...
  it('Zip Creation Test', async () => {
    ...
    const program = (zipFilenameToWrite: string) =>
      Effect.scoped(
        Effect.gen(function* () {
          const zip = yield* ZipService
          yield* zip.create(transferInformationList).pipe(writeStreamToFile(zipFilenameToWrite))
          const entries = yield* zip
            .unarchiveFromFile(zipFilenameToWrite, 'shift-jis')
            .pipe(Stream.runCollect)
  ...
  it('ZipEffect1 Unarchive Test', async () => {
    ...
    const program = (
      zipFilenameToUnarchive: string,
      transferInformationToUnarchive: FileTransportInfo,
    ) =>
      Effect.scoped(
        Effect.gen(function* () {
          const zip = yield* ZipService
          return yield* zip
            .unarchiveFromFile(zipFilenameToUnarchive, 'shift-jis')
            .pipe(zip.extract(transferInformationToUnarchive))
        }),
      )
    await runNodeWithEnvOrThrow(program(zipFilename, transferInformation), ZipService.live)
    extractedFile = path.join(extractDirectory, 'outputREADME.md')
    expect(
      compareFiles(extractedFile, path.join(compressDirectory, 'source/README.md')),
    ).toBeTruthy()
    ...
    expect(
      compareFiles(extractedFile, path.join(compressDirectory, 'source/folder1/email_sender.py')),
    ).toBeTruthy()
    ...
    expect(
      compareFiles(extractedFile, path.join(compressDirectory, 'source/ユーザー噂.py')),
    ).toBeTruthy()
  })
  ...
  it('ZipEffectUnarchive Folder Test', async () => {
    ...
    const program2 = (zipFilenameToUnarchive: string, destinationFolder: string) =>
      Effect.scoped(
        Effect.gen(function* () {
          const zip = yield* ZipService
          return yield* zip
    ...
    await runNodeWithEnvOrThrow(program2(zipFilename, destinationRoot), ZipService.live)
    validateFolders(path.join(compressDirectory, 'source'), destinationRoot)
  })
})
describe('ZipCompare test', () => {
  it('Zip Compare Test', async () => {
    const program = await compareZip({
      ...
      destinationFilename: path.join(compressDirectory, 'compress', 'compare2.zip'),
