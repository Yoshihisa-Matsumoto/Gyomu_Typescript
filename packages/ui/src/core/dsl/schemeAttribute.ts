...
export function buildFormMetaFromStructSchema<TFields extends Fields>(args: {
  ...
  return fields
    .map((f) => {
      ...
      if (name == 'modifiedAt') {
        args.logger?.debug(f, 'digging AST')
      }
      const annotations = getMergedAnnotations(name, f.type, result, args.logger)
      ...
      if (f.type._tag == 'Union') {
        ...
        if (enums.length > 0) validateEnumAttribute(enums, mergeUi, name)
      }
...
const mergeAnnotation = <T extends UIAnnotation>(base: T, override?: UIAnnotationOverride): T => {
  return {
    ...base,
    ...override,
  }
