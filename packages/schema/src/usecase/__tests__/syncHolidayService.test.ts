...
describe('syncHoliday', () => {
  it('正常系：diffが正しく計算される', async () => {
    // repositoryモック
    repoMock.marketHoliday.findByMarket.mockReturnValue(Effect.succeed(mockExisting))
    ...
    const HolidayFetcherMock = Layer.succeed(HolidayFetcher, {
      fetch: () => Effect.succeed(mockIncoming),
    })
  ...
  it('existingがtargetYearsでfilterされる', async () => {
    ...
    repoMock.marketHoliday.synchronizeRecords.mockReturnValue(Effect.succeed({}))
