...
describe('createFileHashEntry', () => {
  it('creates file hash entry', async () => {
    ...
    const result = await Effect.runPromise(
      createFileHashEntry(fileInfo).pipe(Effect.provide(NodeFileSystem.layer)),
    )
  ...
  it('returns different hash when content changes', async () => {
    ...
    const result1 = await Effect.runPromise(
      createFileHashEntry(fileInfo1).pipe(Effect.provide(NodeFileSystem.layer)),
    )

    const result2 = await Effect.runPromise(
      createFileHashEntry(fileInfo2).pipe(Effect.provide(NodeFileSystem.layer)),
    )
